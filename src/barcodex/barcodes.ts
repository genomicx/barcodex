...
const FORMATS: Record<string, FormatDef> = {
    ...
    options: [
      {
        ...
        tooltip:
          'Controls how much of the code can be damaged and still scan. Higher levels make the code larger but more resilient to scratches, smudges, or label wear.',
        type: 'select',
        default: 'M',
        choices: [
  ...
  code39: {
    ...
    validate: (text) => {
      if (!text) return 'Enter text to encode'
      const valid = /^[A-Z0-9\-. $/+%]+$/
      if (!valid.test(text.toUpperCase()))
...
export function get(id: string): FormatDef | null {
  return FORMATS[id] ?? null
}

export function all(): Record<string, FormatDef> {
  return FORMATS
}

export function groups(): FormatGroup[] {
  return GROUPS
}

export function getPlaceholder(id: string): string {
