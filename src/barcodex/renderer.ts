 ...
 */
function addCaption(srcCanvas: HTMLCanvasElement, text: string, scale: number): HTMLCanvasElement {
 ...
 */
function needsCaption(formatDef: FormatDef, showText: boolean): boolean {
...
export function toSVG(
  ...
  if (typeof bwipjs.toSVG === 'function') {
    const opts = buildOptions(formatDef, text, userOpts, showText)
    const { scale: _scale, ...svgOpts } = opts as BwipOptions & { scale?: number }
    void _scale
    let svg: string
    try {
      svg = bwipjs.toSVG(svgOpts)
    } catch (e) {
      throw new Error(friendlyError(e))
    }
